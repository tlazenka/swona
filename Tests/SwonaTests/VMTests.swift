...
@MainActor final class VMTests: XCTestCase {
  ...
  func testNativeFunctionCallWithSingleParameter() throws {
    try evaluator.bind(
      name: "inc",
      value: .function(
        fun1(name: "inc", argType: Type.int, returnType: Type.int) { (value: Value) -> Value in
  ...
  func testNativeFunctionCallWithMultipleParameters() throws {
    try evaluator.bind(
      name: "sub",
      value: .function(
        fun2(name: "sub", argType: Type.int, returnType: Type.int) { (a: Value, b: Value) -> Value in
          a.minus(rhs: b)
        }
      ),
      mutable: false
  ...
  func testRuntimeFunctions() throws {
    ...
    try evaluate(
      code: #"while (!done) { stringArraySet(a, i, "" + i); i = i + 1; if (i==stringArrayLength(a)) done=true }"#
    )
