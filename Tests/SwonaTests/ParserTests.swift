...
@MainActor final class ParserTests: XCTestCase {
  ...
  func testIfAsAnExpression() throws {
    try assertParseExpression(
      source: "1 + if (true) 2 else 3",
      expected: "[Plus [Lit 1] [If [Lit true] [Lit 2] [Lit 3]]]"
    )
    try assertParseExpression(
      source: "if (true) 2 else 3 + 4",
      expected: "[If [Lit true] [Lit 2] [Plus [Lit 3] [Lit 4]]]"
    )
    try assertParseExpression(
      source: "(if (true) 2 else 3) + 4",
      expected: "[Plus [If [Lit true] [Lit 2] [Lit 3]] [Lit 4]]"
    )
  }

  func testUnlessAsAnExpression() throws {
    try assertParseExpression(
      source: "1 + unless (true) 2 else 3",
      expected: "[Plus [Lit 1] [If [Not [Lit true]] [Lit 2] [Lit 3]]]"
    )
    try assertParseExpression(
      source: "unless (true) 2 else 3 + 4",
      expected: "[If [Not [Lit true]] [Lit 2] [Plus [Lit 3] [Lit 4]]]"
    )
    try assertParseExpression(
      source: "(unless (true) 2 else 3) + 4",
      expected: "[Plus [If [Not [Lit true]] [Lit 2] [Lit 3]] [Lit 4]]"
    )
  }
  ...
  func testOperatorPrecedence() throws {
    ...
    try assertParseExpression(
      source: "a + (b == c) + d",
      expected: "[Plus [Plus [Ref a] [== [Ref b] [Ref c]]] [Ref d]]"
    )
    ...
    try assertParseExpression(
      source: "a + b * c + d",
      expected: "[Plus [Plus [Ref a] [Multiply [Ref b] [Ref c]]] [Ref d]]"
    )
    try assertParseExpression(source: "a == b < c", expected: "[== [Ref a] [< [Ref b] [Ref c]]]")
    try assertParseExpression(
      source: "a == b || c == d && e == f",
      expected: "[Or [== [Ref a] [Ref b]] [And [== [Ref c] [Ref d]] [== [Ref e] [Ref f]]]]"
    )
  }
  ...
  func testFunctionDefinition() throws {
    try assertParseFunctionDefinition(
      source: "fun square(x: Int, y: Int): Int = x * x",
      expected:
