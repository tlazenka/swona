...
@MainActor final class LexerTests: XCTestCase {
  ...
  func testLiteralStrings() {
    ...
    assertTokens(
      source: "\"bar \\\"baz\\\" quux\"",
      tokens: Token.literal(value: Value.string(value: "bar \"baz\" quux"))
    )
  }
  ...
  func testMultipleTokens() {
    assertTokens(
      source: "if (foo) \"bar\" else 42",
      tokens: Token.keyword(.if),
      .punctuation(.leftParen),
      .identifier(name: "foo"),
      .punctuation(.rightParen),
      .literal(value: Value.string(value: "bar")),
      .keyword(.else),
      .literal(value: Value.integer(value: 42))
    )
  }
  ...
  func testTokenLocations() throws {
    ...
    XCTAssertTrue(
      [1, 1, 1, 1, 2, 2, 2, 3, 4, 4, 4].elementsEqual(
        locations.map { source.lines().map { String($0) }.firstIndex(of: $0.lineText)! + 1 }
      ),
      "lineTexts"
    )
