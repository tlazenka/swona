  ...
  private func eval(env: ConstantBindingEnv) -> TypedExpression {
    ...
    case let .if(condition, consequent, alternative, type):
      ...
      return .if(
        ...
        consequent: consequent.eval(env: env.child()),
        alternative: alternative?.eval(env: env.child()),
        type: type
      )
    ...
    case let .expressionList(expressions, type):
      ...
      return expressions.singleOrNull()?.eval(env: childEnv)
        ?? .expressionList(expressions: expressions.map { $0.eval(env: childEnv) }, type: type)
    }
  }
}
...
private struct RedundantLoadStoreOptimizer: PeepholeOptimizer {
  ...
  func optimizeWindow(window: [IR]) -> [IR]? {
    ...
    if case let .localFrameIR(localIR1) = first, case let .loadLocal(l1Index, _) = localIR1,
      case let .localFrameIR(localIR2) = second, case let .storeLocal(l2Index, _) = localIR2, l1Index == l2Index
    {
      return []
    } else {
      return nil
...
private struct RedundantLoadOptimizer: PeepholeOptimizer {
  ...
  func optimizeWindow(window: [IR]) -> [IR]? {
    ...
    if case let .localFrameIR(localIR1) = first, case let .storeLocal(l1Index, _) = localIR1,
      case let .localFrameIR(localIR2) = second, case let .loadLocal(l2Index, _) = localIR2, l1Index == l2Index
    {
      return [IR.dup, first]
    } else {
      return nil
