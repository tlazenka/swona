    ...
    for block in allBlocks() {
      ...
      for next in block.next.blocks {
        ...
        if nextDepth == nil {
          ...
          throw InvalidStackUseException(
            "expected \(String(describing: nextDepth)), but got \(endDepth) for \(block) -> \(next)"
          )
        }
      }
    }
...
public class OpCodeTranslator {
  ...
  private func translateBlock(
    block: BasicBlock,
    blockAddresses: [BasicBlock: Int],
    stackDepths: [BasicBlock: Int]
  ) throws -> [OpCode] {
    var ops1 = [OpCode]()
  ...
  func translate(ir: IR, sp: Int) -> OpCode {
    ...
    case let IR.loadArgument(index, name):
      return OpCode.copy(target: sp + 1, source: argumentOffset(index: index), description: "load arg \(name)")
...
class FunctionTranslator {
  ...
  func translateFunction(func: FunctionDefinition, optimize: Bool) throws -> (Type.Function, CodeSegment) {
    ...
    return try (
      Type.Function.function(argumentTypes: `func`.args.map(\.1), returnType: typedExp.type),
      basicBlocks.translateToCode(argumentCount: `func`.args.count)
    )
  }
}
