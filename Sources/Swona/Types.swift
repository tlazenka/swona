...
extension Expression {
  public func typeCheck(env: StaticEnvironment) throws -> TypedExpression {
    ...
    case let .call(`func`, args):
      ...
      guard case let .function(function) = typedFunc.type else {
        throw TypeCheckException(
          errorMessage: "expected function type for call, but got \(typedFunc.type)",
      ...
      case let .function(argumentTypes, returnType):
        ...
        if args.count != expectedArgTypes.count {
          throw TypeCheckException(
            errorMessage: "expected \(expectedArgTypes.count) arguments, but got \(args.count)",
            sourceLocation: location
          )
        }

        let typedArgs = try args.enumerated().map { i, arg in
          try arg.typeCheckExpected(expectedType: expectedArgTypes[i], env: env)
        }
    ...
    case let .assign(variable, expression, location):
      ...
      if !binding.mutable {
        throw TypeCheckException(
          errorMessage: "can't assign to immutable variable \(binding.name)",
          sourceLocation: location
        )
      }
...
extension Expression.Binary {
  func typeCheck(env: StaticEnvironment) throws -> TypedExpression {
    ...
    case let .and(lhs, rhs, _):
      ...
      return TypedExpression.if(
        condition: typedLhs,
        consequent: typedRhs,
        alternative: TypedExpression.lit(value: Value.bool(value: false), type: .boolean),
        type: Type.boolean
      )
    case let .or(lhs, rhs, _):
      ...
      return TypedExpression.if(
        condition: typedLhs,
        consequent: TypedExpression.lit(value: Value.bool(value: true), type: .boolean),
        alternative: typedRhs,
        type: Type.boolean
      )
    case let .relational(op, lhs, rhs, location):
      ...
      if !l.type.supports(op: op) {
        throw TypeCheckException(
          errorMessage: "operator \(op) is not supported for type \(l.type)",
          sourceLocation: location
        )
      }
  ...
  func typeCheckMatching(
    env: StaticEnvironment,
    lhs: Expression,
    rhs: Expression,
    location: SourceLocation
  ) throws -> (TypedExpression, TypedExpression) {
    let typedLhs = try lhs.typeCheck(env: env)
    let typedRhs = try rhs.typeCheck(env: env)

    if typedLhs.type != typedRhs.type {
      throw TypeCheckException(
        errorMessage: "lhs type \(typedLhs.type) did not match rhs type \(typedRhs.type)",
        sourceLocation: location
      )
    }
