...
public struct DataSegment: CustomStringConvertible {
  ...
  public var description: String {
    "[\(bindings.prefix(10).map { $0!.description }.joined(separator: ", "))]"
  }
}
...
public class Evaluator {
  ...
  public init(trace: Bool = false) {
    functionTranslator = FunctionTranslator(env: globalTypeEnvironment)
    self.trace = trace
  }
  ...
  public func dump(code: String) throws -> String {
    ...
    if case let .ref(bindingReference) = exp, case .function = exp.type {
  ...
  public func bindFunction(func: FunctionDefinition) throws {
    ...
    if let returnType = `func`.returnType {
      binding = try globalTypeEnvironment.bind(
        name: `func`.name,
        type: Type.function(.function(argumentTypes: `func`.args.map(\.1), returnType: returnType)),
        mutable: false
      ).binding
    }
    ...
    do {
      ...
      globalData[binding!.index] = Value.function(
        .compound(address: address, codeSize: code.size, name: `func`.name, signature: signature)
      )
    } catch {
      if binding != nil {
  ...
  private func parseAndTypeCheck(code: String) throws -> TypedExpression {
    try parseExpression(code: code).typeCheck(env: globalTypeEnvironment)
  }
  ...
  public func evaluateSegment(segment: CodeSegment) throws -> Value {
    ...
    while run {
      ...
      case .call: try evalCall(op: op, state: &state)
