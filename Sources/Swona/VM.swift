...
public class Evaluator {
    ...
    public func bindFunction(func: FunctionDefinition) throws {
        ...
        if let returnType = `func`.returnType {
            binding = try globalTypeEnvironment.bind(name: `func`.name, type: Type.function(.function(argumentTypes: `func`.args.map(\.1), returnType: returnType)), mutable: false).binding
        }
