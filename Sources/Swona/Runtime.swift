...
public func fun1(
  ...
  argType: Type,
  returnType: Type,
  func: @escaping (Value) throws -> Value
) -> Value.Function {
  let signature = Type.Function.function(argumentTypes: [argType], returnType: returnType)
  return .native(
    func: FunctionReference(function: { args in try `func`(args.single()) }),
    name: name,
    signature: signature
  )
}
...
public func fun1(name: String, argType: Type, func: @escaping (Value) throws -> Value) -> Value.Function {
  fun1(name: name, argType: argType, returnType: .unit, func: `func`)
}
...
public func fun2(
  ...
  fun2(name: name, argType1: argType, argType2: argType, returnType: returnType, func: `func`)
}
...
public func fun3(
  ...
  argType3: Type,
  returnType: Type,
  func: @escaping (Value, Value, Value) throws -> Value
) -> Value.Function {
  ...
  return .native(
    func: FunctionReference(function: { args in
      ...
      return try `func`(args[0], args[1], args[2])
    }),
    name: name,
    signature: signature
  )
...
public func registerRuntimeFunctions(evaluator: Evaluator) throws {
  ...
  try evaluator.bindFunction(
    func: fun2(
      name: "stringArrayOfSize",
      argType1: .int,
      argType2: .string,
      returnType: .array(elementType: .string),
      func: {
        size,
        initialValue in
        ...
        return .array(
          elements: ArrayReference(array: [Value](repeating: .string(value: i), count: s)),
          elementType: .string
        )
      }
    )
  ...
  try evaluator.bindFunction(
    func: fun1(
      name: "stringArrayLength",
      argType: .array(elementType: .string),
      returnType: .int,
      func: {
  ...
  try evaluator.bindFunction(
    func: fun2(
      name: "stringArrayGet",
      argType1: .array(elementType: .string),
      argType2: .int,
      returnType: .string,
      func: {
        array,
        index in
        guard case let .array(elements, elementType) = array, elementType == .string, case let .integer(i) = index
        else {
