...
public class Parser {
  ...
  private func parseExpression4() throws -> Expression {
    ...
    while lexer.hasMore {
      ...
      if try lexer.readNextIf(token: .operator(.lessThan)) {
        ...
        exp = try Expression.binary(
          .relational(op: .lessThanOrEqual, lhs: exp, rhs: parseExpression5(), location: location)
        )
      } else if try lexer.readNextIf(token: .operator(.greaterThan)) {
        exp = try Expression.binary(
          .relational(op: .greaterThan, lhs: exp, rhs: parseExpression5(), location: location)
        )
      } else if try lexer.readNextIf(token: .operator(.greaterThanOrEqual)) {
        exp = try Expression.binary(
          .relational(op: .greaterThanOrEqual, lhs: exp, rhs: parseExpression5(), location: location)
        )
      } else {
        return exp
      }
  ...
  private func parseUnless() throws -> Expression {
    ...
    return Expression.if(
      condition: .not(exp: condition, location: location),
  ...
  private func parseLiteral() throws -> Expression {
    ...
    guard case let Token.literal(value) = tokenInfo.token else {
      throw SyntaxErrorException(
        errorMessage: "expected token of type Token.identifier, but got \(tokenInfo.token.description)",
        sourceLocation: tokenInfo.location
      )
    }
